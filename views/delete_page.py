...
class DeletePage(View):
    ...
    def create_widgets(self):
        ...
        tb.Button(self.top4, text="Delete Information", command=self.funny, bootstyle=INFO).pack(side=RIGHT,
                                                                                                ipady=6,
                                                                                                ipadx=20,
                                                                                                padx=29,
                                                                                                pady=10)
    ...
    def profile(self):
        ...
        req.delete(f'{self.app.url}fruits/{task_id}',
    ...
    def balls(self):
        task_id = str(self.id.get())
        req.delete(f'{self.app.url}tasks/{task_id}?auth={self.app.token["access_token"]}',
                          headers={'Authorization': f'string {self.app.token["access_token"]}',
                                   'Content-Type': 'application/json'})
        self.app.user_page()
    ...
    def funny(self):
        task_id = str(self.id.get())
        req.delete(f'{self.app.url}profiles/{task_id}?auth={self.app.token["access_token"]}',
                          headers={'Authorization': f'string {self.app.token["access_token"]}',
                                   'Content-Type': 'application/json'})
        self.app.user_page()

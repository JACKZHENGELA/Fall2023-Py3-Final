...
class GetPage(View):
    ...
    def pain(self):
        ...
        nuts = req.get(f'{self.app.url}fruits/{task_id}', headers={'Content-Type': 'application/json'}).json()
        self.app.views.get('user_orders').color.configure(text=f"Color: {nuts['color']}")
        self.app.views.get('user_orders').fruit_type.configure(text=f"Fruit Type: {nuts['fruit_type']}")
        self.app.views.get('user_orders').grown.configure(text=f"Grown?: {nuts['grown']}")
        self.app.views.get('user_orders').planter.configure(text=f"Size: {nuts['size']}")
    ...
    def balls(self):
        task_id = str(self.id.get())
        nuts = req.get(f'{self.app.url}tasks/{task_id}?auth={self.app.token["access_token"]}',
                          headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                                   'Content-Type': 'application/json'}).json()
        self.app.views.get('user_orders').color.configure(text=f'Title: {nuts["title"]}')
        self.app.views.get('user_orders').fruit_type.configure(text=f'Description: {nuts["description"]}')
        self.app.views.get('user_orders').grown.configure(text=f'Priority: {nuts["priority"]}')
        self.app.views.get('user_orders').planter.configure(text=f'Completion: {nuts["complete"]}')
        self.app.views.get('user_orders').size.configure(text="")
        self.app.user_orders()

    def funny(self):
        task_id = str(self.id.get())
        haha = req.get(f'{self.app.url}profiles/{task_id}?auth={self.app.token["access_token"]}',
                          headers={'Authorization': f'string {self.app.token["access_token"]}',
                                   'Content-Type': 'application/json'}).json()
        self.app.views.get('user_orders').color.configure(text=f'gpa: {haha["gpa"]}')
        self.app.views.get('user_orders').fruit_type.configure(text=f'School: {haha["school"]}')
        self.app.views.get('user_orders').grown.configure(text=f'Gender: {haha["gender"]}')
        self.app.views.get('user_orders').planter.configure(text=f'Have Pets?: {haha["have_pet"]}')
        self.app.views.get('user_orders').size.configure(text="")
        self.app.user_orders()

...
class CreatePage(View):
    ...
    def create_widgets(self):
        ...
        tb.Label(self.top3, text="Let's Plan Something For Today!", bootstyle="inverse-primary", font=('Helvetica', 30)).pack(side=TOP, pady=10)
        self.top4 = tb.Frame(self.top3, bootstyle=PRIMARY)
        self.top4.pack(side=TOP, pady=10)
        tb.Button(self.top4, text="Plant a plant", command=self.ball_cancer, bootstyle=INFO).pack(side=LEFT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        tb.Button(self.top4, text="Feed animals", command=self.touch_grass, bootstyle=INFO).pack(side=RIGHT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        ...
        tb.Label(self.bottom5, text="Completion (true or false):", bootstyle="inverse-primary", font=('Helvetica', 15)).pack(side=LEFT,
                                                                                                          pady=10)
        tb.Entry(self.bottom5, bootstyle=INFO, textvariable=self.completion).pack(side=RIGHT, pady=10)
    ...
    def ball_cancer(self):
        data = {"completion": self.completion.get(), "description": "Plant a plant", "priority": 5,"title": "Plant"}
        req.post(f'{self.app.url}tasks?auth={self.app.token["access_token"]}',
                 headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                          'Content-Type': 'application/json'}, json=data)


    def touch_grass(self):
        data = {"completion": self.completion.get(), "description": "Feed animals", "priority": 1,"title": "Animals"}
        req.post(f'{self.app.url}tasks?auth={self.app.token["access_token"]}',
                 headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                          'Content-Type': 'application/json'}, json=data)

...
class CreatePage(View):
    ...
    def create_widgets(self):
        ...
        tb.Button(self.top4, text="Have Nolan Get Ball Cancer", command=self.ball_cancer, bootstyle=INFO).pack(side=LEFT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        tb.Button(self.top4, text="Cure Nolan's Ball Cancer by letting him touch grass", command=self.touch_grass, bootstyle=INFO).pack(side=RIGHT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
    ...
    def ball_cancer(self):
        data = {"completion": self.completion.get(), "description": "Plant a tumor in Nolan's Balls", "priority": 5,"title": "Get Ball cancer"}
    ...
    def touch_grass(self):
        data = {"completion": self.completion.get(), "description": "Cure Nolan's Ball Cancer", "priority": 1,"title": "ball cancer curing"}
        req.post(f'{self.app.url}tasks?auth={self.app.token["access_token"]}',
                 headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                          'Content-Type': 'application/json'}, json=data)

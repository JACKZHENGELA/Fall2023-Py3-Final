...
class CPage(View):
    ...
    def create_widgets(self):
        ...
        tb.Label(self.top3, text="Create A Farmer Profile", bootstyle="inverse-primary", font=('Helvetica', 30)).pack(side=TOP, pady=10)
        self.top4 = tb.Frame(self.top3, bootstyle=PRIMARY)
        self.top4.pack(side=TOP, pady=10)
        tb.Button(self.top4, text="Male", command=self.ball_cancer, bootstyle=INFO).pack(side=LEFT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        tb.Button(self.top4, text="Female", command=self.touch_grass, bootstyle=INFO).pack(side=RIGHT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
    ...
    def ball_cancer(self):
        ...
        self.create_toast("You did it!", f"You made a male farmer!")
    ...
    def touch_grass(self):
        ...
        req.post(f'{self.app.url}profiles?auth={self.app.token["access_token"]}',
                 headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                          'Content-Type': 'application/json'}, json=data)
        self.create_toast("You did it!", f"You made a female farmer!")

...
class OrderPage(View):
    ...
    def create_widgets(self):
        ...
        tb.Button(self.top4, text="Buy Nolan Wang fruit", command=self.ball_cancer, bootstyle=INFO).pack(side=LEFT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        tb.Button(self.top4, text="Buy jack wang fruit", command=self.touch_grass, bootstyle=INFO).pack(side=RIGHT,
                                                                                          ipady=6,
                                                                                          ipadx=20,
                                                                                          padx=29, pady=10)
        ...
        tb.Label(self.bottom5, text="Grown (true or false):", bootstyle="inverse-primary", font=('Helvetica', 15)).pack(side=LEFT,
                                                                                                          pady=10)
        tb.Entry(self.bottom5, bootstyle=INFO, textvariable=self.growth).pack(side=RIGHT, pady=10)
    ...
    def ball_cancer(self):
        data = {"fruit_type": "Nolan Wang Fruit", "color": "Red", "size": 1, "grown": self.growth.get()}
        req.post(f'{self.app.url}fruits?auth={self.app.token["access_token"]}',
                headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                         'Content-Type': 'application/json'}, json=data)


    def touch_grass(self):
        data = {"fruit_type": "Jack Wang Fruit",
                "color": "Red",
                "size": 1,
                "grown": self.growth.get()}
        req.post(f'{self.app.url}fruits?auth={self.app.token["access_token"]}',
                 headers={'Authorization': f'Bearer {self.app.token["access_token"]}',
                          'Content-Type': 'application/json'}, json=data)
